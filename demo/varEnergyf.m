...
function varEnergyf(n, idxRepet, nSamplesBatch, idxT, Ts)
...
ticTime = now;
 load('/home/marcel/criticalityIterScaling/results/final/lambdaHatFFF.mat') % got to transfer a couple of lambdas to the cluster,
  % gives 1x10 cell 'lambdaHatFFF'               % ideally those fit to the full model (h,J,V) so far
if idxRepet <= size(lambdaHatFFF{n},2)   
 lambda = lambdaHatFFF{n}(:,idxRepet);
else
 warning('\lambda for desired idxRepet not found - using first one found')
 tmp = lambdaHatFFF{n}(:,1);
 for j = 2:size(lambdaHatFFF{n},2)
  if max(abs(lambdaHatFFF{n}(:,j))) > 0
    tmp = lambdaHatFFF{n}(:,j);
  end
 end
...
for i = 1:N
   ...
   if mod(i,20)==0
    ...
    save(['/home/marcel/criticalityIterScaling/results/energy/VarEnf', num2str(n), ...
