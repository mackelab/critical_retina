function runMaxEntSimData(n, idxRepet, fitoptions, beta, a,tau, hJV, fname)
...
if nargin < 4 || isempty(beta)
 beta = 0.00001*ones( n*(n+3)/2 + 1,1); % very weak regularization
end

if nargin > 4 && ~isempty(a) && ~isempty(tau)
...
for i = 1:length(idxSubsamples)
 if size(idxSubsamples{i},1)==n
     Efx = Efx{i}(:,idxRepet); % discard all other runs
     break
 end
end
...
for i = 1:size(Efx,2)
 ...
 if isnan(fitoptions.lambda0) % each run needs its own initialization
  ...
  fitoptions.lambda0(1:n) = log(Efx(1:n,i)./(1-Efx(1:n,i)));
 ...
 [lambdaHat(:,i), fD{i}] = iterScaling(Efx(:,i), fitoptions, beta, ...
                                       [fname, 'idxRepet', num2str(i)], ...
                                       ifSave, hJV, ifbwVK);
 save([folder,fname,'final.mat'], 'lambdaHat', 'fD', 'beta', 'n', 'idxRepet', 'fitoptions');
