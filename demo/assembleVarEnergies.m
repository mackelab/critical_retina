function assembleVarEnergies(ns, idxRepet, idxTs)
...
if length(ns) == 1
 ...
 for i = idxRepet
   names = strsplit(ls(['VarEn', num2str(10*ns), 'idxRepet', num2str(i), 'T*']));
   if strcmp(names{end}, '')
 ...
 save(['VarEn', num2str(10*ns), 'HeatTraces.mat'], 'MoEs', 'varE', 'ns', 'idxRepet', 'Ts')
 ...
 varE  = zeros(length(ns), length(idxRepet), length(idxTs));
  for n = ns
   for i = idxRepet
     names = strsplit(ls(['VarEn', num2str(10*n), 'idxRepet', num2str(i), 'T*']));
     if strcmp(names{end}, '')
       names = names(1:end-1); % last one at leat on the on the cluster always is the empty string ''
     elseif strcmp(names{1}, '')
       names = names(2:end);
 ...
 save(['VarESimHeatTraces.mat'], 'MoEs', 'varE', 'ns', 'idxRepet', 'Ts')
end
