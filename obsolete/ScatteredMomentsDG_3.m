...
basename='~/CopyMPI/MaxEnt/Data/ScatteredMomentsDG_3';
thisdir=pwd;
...
callfile=mfilename;
dimo=[10]; %use Ising model with 2, 3 5, 10, 15 dimensions

%means=0;;
scatterstds=[0,1,.5,.25,.1,2]; %standard devation of Js for population size 1, for population size n, std is divided by sqrt(n)
numrethrowparams=1000; %for each experiment, try 5 random throws of parameters J
...
for kk=1:numrethrowparams
    for k=1:numel(scatterstds);
        ...
        p.started=nan;
        p.finished=nan;
        p.filename=sprintf('%s/Res_%03d_%s',basename,k,datestr(now,30));
        try
            save(p.filename,'p')
        end
